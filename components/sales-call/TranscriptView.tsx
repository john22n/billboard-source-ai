  ...
  ({ transcripts, interimTranscript, interimSpeaker, twilioReady }, ref) => {
    ...
    return (
      ...
      >
          ...
          return (
            ...
            >
              <div className={`max-w-[85%] ${isAgent ? 'order-2' : ''}`}>
                <div
                  className={`flex items-center gap-1.5 mb-1 ${isAgent ? 'justify-end' : ''}`}
                >
                  <span
                    className={`text-xs font-medium px-2 py-0.5 rounded-full text-white ${styles.badge}`}
                  >
                    {styles.label}
                  </span>
                ...
                <div
                  className={`rounded-lg p-3 shadow-sm border ${styles.bubble}`}
                >
        ...
        {interimTranscript && (
          <div
            className={`mb-3 flex ${interimSpeaker === 'agent' ? 'justify-end' : 'justify-start'}`}
          >
            <div className="max-w-[85%]">
              <div
                className={`flex items-center gap-1.5 mb-1 ${interimSpeaker === 'agent' ? 'justify-end' : ''}`}
              >
                <span
                  className={`text-xs font-medium px-2 py-0.5 rounded-full text-white ${interimSpeaker === 'agent' ? 'bg-blue-400' : 'bg-emerald-400'}`}
                >
        ...
        {transcripts.length === 0 && !interimTranscript && (
          <div className="flex flex-col items-center justify-center h-full text-center py-16">
            ...
            <p className="text-slate-300 text-sm mt-1">
              {twilioReady
                ? 'Accept a call to start transcribing'
                : 'Connect Twilio to start transcribing'}
            </p>
          </div>
        )}
      </div>
