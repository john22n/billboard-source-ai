...
export function PricingPanel({
  ...
  useEffect(() => {
    ...
    console.log(
      `🔍 Effect check - Market #${activeMarketIndex + 1}, location: "${location}"`,
    )
    ...
    if (!alreadyFetched) {
      // Debounce the fetch
      console.log(
        `⏱️ Scheduling fetch for Market #${activeMarketIndex + 1}: ${location}`,
      )
  ...
  const parsePricingData = (): PricingCard[] => {
    ...
    const avgViewsMatch = contextToUse.match(
      /Average\s*Daily\s*Views:\s*([\d,]+)/i,
    )
    const fourWeekMatch = contextToUse.match(
      /4-Week\s*Price\s*Range:\s*(\$[\d,]+\s*-\s*\$[\d,]+)/i,
    )
    const marketRangeMatch = contextToUse.match(/Market\s*Range:\s*([^\n.]+)/i)
    const hasGeneralPricing = /General\s*Pricing:/i.test(contextToUse)

    const marketNameMatch = contextToUse.match(
      /Market(?:\s*Name)?:\s*([^,\n]+)/i,
    )
    ...
    // PRIORITY 4: BLACK - has General Pricing with market size tiers
      ...
      const generalPricingIndex = contextToUse
        .toLowerCase()
        .indexOf('general pricing:')
      ...
      const generalRanges = rangesAfterGeneral
        ? [...new Set(rangesAfterGeneral)]
        : []
  ...
  const getCardColors = (type: PricingCard['type']) => {
    switch (type) {
      case 'avg-views':
        return { primary: '#2563eb', text: '#2563eb' }
      case 'four-week-only':
        return { primary: '#16a34a', text: '#16a34a' }
      case 'market-range':
        return { primary: '#7c3aed', text: '#7c3aed' }
      case 'general-range':
        return { primary: '#000000', text: '#000000' }
      default:
        return { primary: '#000000', text: '#000000' }
    }
  }
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto min-h-0 space-y-3 sm:space-y-4">
            ...
            <>
              {isLoadingData && (
                <div className="flex flex-col items-center justify-center py-6 sm:py-12">
                  ...
                  <p className="text-slate-600 font-medium mt-2 sm:mt-4 text-xs text-center">
                  ...
                  return (
                    <div key={index}>
                      <div className="text-center py-2 sm:py-3 border-b-2 border-black">
                        <h2
                          className="text-lg sm:text-2xl xl:text-3xl font-bold"
                      ...
                      <div
                        className="text-center py-1.5 sm:py-3 border-l-2 border-r-2 border-black text-white font-bold text-sm sm:text-lg xl:text-xl"
                      ...
                      {card.type === 'avg-views' && card.subtitle && (
                        <>
                          <div
                            className="text-center text-sm sm:text-lg xl:text-xl py-1 sm:py-2 border-l-2 border-r-2 border-black text-white font-bold mt-1.5 sm:mt-3"
                            style={{ backgroundColor: colors.primary }}
                          >
                            {card.subtitle}
                          </div>
                          <div
                            className="text-center text-lg sm:text-2xl xl:text-3xl py-2 sm:py-5 border-2 border-t-0 border-black font-bold bg-white"
                            style={{ color: colors.text }}
                          >
                            {card.subtitleData}
                          </div>
                      ...
                      {card.note && (
                        <div className="px-2 sm:px-3 pt-1 sm:pt-2 pb-1.5 sm:pb-3 text-center">
                          <p
                            className="text-[9px] sm:text-xs italic font-medium"
                            style={{ color: colors.text }}
                          >
                            {card.note}
                          </p>
              ...
              {!isLoadingData &&
                ...
                pricingCards.length === 0 && (
                  <div className="bg-white rounded-lg p-3 sm:p-4 shadow-sm border border-slate-200">
                    <pre className="whitespace-pre-wrap font-sans text-slate-700 leading-relaxed text-[10px] sm:text-xs">
                      {currentMarketContext}
                    </pre>
                  </div>
                )}
          ...
          {activeTab === 'details' && (
            <>
              <h3 className="text-lg sm:text-2xl font-black text-slate-800 mb-2 sm:mb-3 border-b-2 border-slate-200 pb-1 sticky top-0 bg-white">
                Pricing Details
              </h3>
              {parsedDetails.length > 0 ? (
                <div className="space-y-1 sm:space-y-2">
                  {parsedDetails.map((field, idx) => (
                    <div
                      key={idx}
                      className="py-0.5 sm:py-1.5 border-b border-slate-100 last:border-b-0"
                    >
                      <span className="font-bold text-slate-800 text-[10px] sm:text-sm">
                        {field.label}
                      </span>
                      <span className="font-semibold text-blue-600 text-[10px] sm:text-sm ml-1">
                        {field.value}
                      </span>
                    </div>
                  ))}
