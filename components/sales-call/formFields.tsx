...
export const FieldInput = memo(function FieldInput({
  ...
  if (hasValidationError && (!value || value.trim() === '')) {
    inputClass +=
      ' ring-2 ring-red-500 ring-offset-1 border-red-500 animate-pulse'
  }
...
export const PhoneInput = memo(function PhoneInput({
  ...
  if (hasPhone) {
    if (phoneVerified) {
      colorClass =
        'bg-green-100 border-green-600 shadow-lg ring-2 ring-green-400 focus:border-green-700 focus:ring-green-500 focus:ring-offset-1'
      wrapperClass = 'relative'
    } else if (twilioPhonePreFilled) {
      colorClass =
        'bg-yellow-50 border-yellow-500 focus:border-yellow-600 focus:ring-yellow-400'
...
export const ContactFieldInput = memo(function ContactFieldInput({
  ...
  const baseClassName =
    'h-9 sm:h-10 text-xs sm:text-sm border-2 border-black rounded transition-colors'
  let inputClass =
    field === 'name'
...
export const MarketFieldTextarea = memo(function MarketFieldTextarea({
  ...
  const inputClass = getInputClass(
    value,
    'text-xs sm:text-sm resize-none border-2 border-black rounded transition-colors',
  )
...
export const MultiSelectButtonGroup = memo(function MultiSelectButtonGroup({
  ...
  const fieldValueRaw = useFormStore((s) =>
    marketOrContactIndex === 0 ? s.fields[field] : null,
  )
  const aiSuggestions = (() => {
...
export const DecisionMakerButtonGroup = memo(function DecisionMakerButtonGroup({
  ...
  const confirmedValue = useFormStore(
    (s) => s.confirmedDecisionMakers[contactIndex] ?? null,
  )
  const setConfirmedDecisionMaker = useFormStore(
    (s) => s.setConfirmedDecisionMaker,
  )
  const updateContactField = useFormStore((s) => s.updateContactField)
  ...
  return (
    <div
      className={`flex gap-1 sm:gap-2 flex-wrap sm:flex-nowrap ${className}`}
    >
      {options.map((option) => {
        let bgClass = 'bg-red-100 border-black'
        if (confirmedValue === option.value) {
...
export const BoardTypeButtonGroup = memo(function BoardTypeButtonGroup({
  ...
  const confirmedValue = useFormStore(
    (s) => s.confirmedBoardTypes[marketIndex] ?? null,
  )
  ...
  return (
    <div
      className={`flex gap-1 sm:gap-1.5 w-full overflow-hidden ${className}`}
    >
      {options.map((option) => {
        let bgClass = 'bg-red-100 border-black'
        if (confirmedValue === option.value) {
...
export const DurationButtonGroup = memo(function DurationButtonGroup({
  ...
  const campaignLengthRaw = useFormStore((s) =>
    marketIndex === 0 ? s.fields.campaignLength : null,
  )
  const aiSuggestions = (() => {
    if (marketIndex !== 0 || !campaignLengthRaw) return []
    if (Array.isArray(campaignLengthRaw))
      return campaignLengthRaw.flat() as string[]
  ...
  return (
    <div
      className={`flex gap-0.5 sm:gap-1 max-w-full min-w-0 overflow-hidden ${className}`}
    >
      {options.map((option) => {
        const isConfirmed = confirmedSelections.includes(option.value)
        const isAISuggested = aiSuggestions.includes(option.value)
        ...
        return (
          <div
            key={option.value}
            className="flex flex-col text-center items-center flex-1 min-w-0"
          >
            <button
              onClick={() => handleClick(option.value)}
...
export const SendOverButtonGroup = memo(function SendOverButtonGroup({
  ...
  const sendOverRaw = useFormStore((s) =>
    contactIndex === 0 ? s.fields.sendOver : null,
  )
  const aiSuggestions = (() => {
  ...
  const handleClick = useCallback(
    (value: string) => {
      ...
      if (contactIndex === 0) {
        updateField(
          'sendOver',
          newSelections as ('Avails' | 'Panel Info' | 'Planning Rates')[],
        )
      }
    },
  ...
  return (
    ...
    >
      {options.map((option) => {
        const isConfirmed = confirmedSelections.includes(option.value)
        const isAISuggested = aiSuggestions.includes(
          option.value as 'Avails' | 'Panel Info' | 'Planning Rates',
        )
...
export const BallparkInput = memo(function BallparkInput({
  ...
  const inputClass = getInputClass(
    ballpark,
    'h-9 sm:h-10 flex-1 sm:w-32 md:w-40 xl:w-64 text-xs sm:text-sm border-2 border-black rounded transition-colors',
  )
