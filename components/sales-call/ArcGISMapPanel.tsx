...
export function ArcGISMapPanel({ initialLocation }: ArcGISMapPanelProps) {
  ...
  useEffect(() => {
    ...
    const setupPopups = async () => {
      ...
      view.map.allLayers.forEach((layer: any, index: number) => {
        console.log(
          `Layer ${index}: "${layer.title}" | Type: ${layer.type} | Visible: ${layer.visible} | Loaded: ${layer.loaded}`,
        )
        if (layer.source) {
      ...
      view.map.allLayers.forEach((layer: any) => {
        ...
        if (layer.renderer) {
          ...
          // "All boards no vendor icons" layer → black [0, 0, 0, 255]
            ...
            layerTitle.includes('all boards') &&
            layerTitle.includes('no vendor')
          ) {
            ...
            if (layer.queryFeatureCount) {
              layer
                ...
                .then((count: number) => {
                  console.log(
                    `  - Feature count for "${layer.title}": ${count}`,
                  )
                })
                .catch((err: Error) => {
  ...
  useEffect(() => {
    ...
    // Mirror the last external location to avoid repeating this imperative map move.
  ...
  useEffect(() => {
    if (!isLoaded || !containerRef.current) return
    const layerListContainer = containerRef.current.querySelector(
