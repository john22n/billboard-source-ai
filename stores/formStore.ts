  ...
  updateFromAI: (data) => {
    ...
    const isDifferentValue = (
      ...
      if (typeof currentVal === 'string' && typeof newVal === 'string') {
        ...
        if (
          newNorm.includes(currentNorm) &&
          newNorm.length > currentNorm.length
        ) {
          return true
        }
        ...
        if (
          ...
          overlap.length < currentWords.length * 0.5
        ) {
          return true
        }
    ...
    for (const [key, value] of Object.entries(data)) {
      ...
      // But still skip if value hasn't actually changed
...
export const selectField =
  <K extends FormFieldKey>(field: K) =>
  (state: FormStore) =>
    state.fields[field]

// Select multiple fields
export const selectFields =
  <K extends FormFieldKey>(fields: K[]) =>
  (state: FormStore) => {
...
export const selectIsRecentlyChanged = (field: string) => (state: FormStore) =>
  state.recentlyChangedFields.has(field)
...
export const selectIsFieldLocked = (field: string) => (state: FormStore) =>
  state.lockedFields.has(field)

// ✅ Check if a field was edited by user specifically
export const selectIsUserEdited = (field: string) => (state: FormStore) =>
  state.userEditedFields.has(field)
...
export const selectCurrentMarket = (state: FormStore) => {
  ...
  return (
    state.additionalMarkets[state.activeMarketIndex - 1] ?? {
      targetCity: '',
      state: '',
      targetArea: '',
      startMonth: '',
...
export const selectCurrentContact = (state: FormStore) => {
  if (state.activeContactIndex === 0) {
    return {
      ...
      sendOver: (Array.isArray(state.fields.sendOver)
        ? state.fields.sendOver
        : []
      ).filter(
        (s): s is 'Avails' | 'Panel Info' | 'Planning Rates' => s !== undefined,
      ),
    }
  }
  return (
    state.additionalContacts[state.activeContactIndex - 1] ?? {
      id: '',
      name: '',
      position: '',
      phone: '',
