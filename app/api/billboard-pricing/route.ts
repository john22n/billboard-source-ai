...
/**
 * Use AI SDK to extract structured location data from transcript
 */
async function extractLocationInfo(transcript: string): Promise<ExtractedLocation> {
  try {
    const { object } = await generateObject({
      ...
      schema: z.object({
        cities: z.array(z.string()).describe('Array of city names mentioned'),
        states: z.array(z.string()).describe('Array of state names or abbreviations (normalized to 2-letter codes like TX, CA, AL)'),
        counties: z.array(z.string()).describe('Array of county names (without "County" suffix)'),
        confidence: z.enum(['high', 'medium', 'low']).describe('Confidence level: high if city AND state mentioned, medium if only state/county, low if ambiguous'),
      }),
      prompt: `Extract cities, states, and counties from this conversation.
...
Transcript: ${transcript}`,
    ...
    return {
      ...
      confidence: object.confidence || 'low',
    };
  } catch (error) {
...
/**
 * Extract campaign preferences from transcript using AI SDK
...
async function extractCampaignPreferences(transcript: string): Promise<CampaignPreferences> {
  try {
    const { object } = await generateObject({
      model: openai('gpt-4o-mini'),
      schema: z.object({
        desiredLength: z.enum(['12-week', '24-week', '52-week', 'all']).nullable().describe('Desired campaign length'),
        billboardTypes: z.array(z.enum(['static-bulletin', 'static-poster', 'digital', 'all'])).describe('Types of billboards desired'),
        confidence: z.enum(['high', 'medium', 'low']).describe('Confidence level of extraction'),
      }),
      prompt: `Analyze this conversation to extract billboard campaign preferences.
...
Transcript: ${transcript}`,
    ...
    return {
      desiredLength: object.desiredLength || 'all',
      billboardTypes: object.billboardTypes || ['all'],
      confidence: object.confidence || 'low',
    };
  } catch (error) {
...
/**
 * Query billboard locations using semantic search with AI SDK
 */
