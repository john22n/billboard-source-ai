...
const billboardLeadSchema = z.object({
  ...
  leadType: z
    .enum([
      LeadSentiment.AVAILER,
      LeadSentiment.PANEL_REQUESTER,
      LeadSentiment.TIRE_KICKER,
    ])
    .nullable().describe(`Caller intent classification:
...
- "${LeadSentiment.TIRE_KICKER}": Low intent, just browsing ("Just curious", "Maybe someday")`),
  ...
  typeName: z
    ...
    .nullable().describe(`Business classification (pick ONE):
...
- "${LeadType.PERSONAL}": Individual (not a business)`),
  ...
  businessName: z
    ...
    .nullable()
    .describe(
      "Industry/category: 'HVAC', 'Restaurant', 'Law Firm' for business; 'Governor', 'Mayor' for political; 'Food Bank' for nonprofit",
    ),
  ...
  entityName: z
    ...
    .nullable()
    .describe(
      "Official business/organization name: 'Bob's HVAC', 'Committee to Elect Jane Doe', 'Austin Food Bank'",
    ),

  // === BILLBOARD EXPERIENCE ===
  billboardsBeforeYN: z
    .enum(['Y', 'N'])
    .nullable()
    .describe("Has caller used billboards before? 'Y' or 'N' only"),

  billboardsBeforeDetails: z
    .string()
    .nullable()
    .describe(
      "If billboardsBeforeYN is 'Y': describe experience. If 'N': use 'None'. If not discussed: null",
    ),
  ...
  billboardPurpose: z
    ...
    .nullable().describe(`Primary advertising goal (pick ONE):
...
- "${BillboardPurpose.POLITICAL}": Campaign messaging, vote for X`),
  ...
  accomplishDetails: z
    ...
    .nullable()
    .describe(
      'Additional context about goals not captured by billboardPurpose (secondary goals, specifics)',
    ),
  ...
  targetAudience: z
    ...
    .nullable()
    .describe(
      "Who they want to reach: 'Homeowners', 'Commuters on I-35', 'Families', 'Young professionals'",
    ),

  // === BUSINESS DETAILS ===
  hasMediaExperience: z
    .string()
    .nullable()
    .describe(
      "Other advertising they do: 'Facebook ads', 'Radio', 'TV'. Use 'No' if they explicitly say none. Null if not discussed.",
    ),
  ...
  website: z
    ...
    .nullable()
    .describe(
      "Website URL if mentioned. Use 'No' if they explicitly say they don't have one. Null if not discussed.",
    ),
  ...
  targetArea: z
    ...
    .nullable()
    .describe(
      "Specific roads/highways/areas: 'I-35', 'Highway 183', 'Downtown', 'North side'",
    ),
  ...
  campaignLength: z
    .array(z.enum(['1 Mo', '3 Mo', '6 Mo', '12 Mo', 'TBD']))
    .nullable()
    .describe(
      "Duration(s) - include ALL mentioned: ['3 Mo'] for single, ['3 Mo', '6 Mo'] for '3 to 6 months' range",
    ),

  boardType: z
    .enum(['Static', 'Digital', 'Both'])
    .nullable()
  ...
  phone: z
    ...
    .nullable()
    .describe(
      'Phone from CALLER only (not rep). Format: (XXX) XXX-XXXX. Listen for spelled-out digits.',
    ),
  ...
  email: z
    ...
    .nullable()
    .describe(
      "Email with @ symbol. Convert 'john at company dot com' to 'john@company.com'. Null if not mentioned.",
    ),

  // === DECISION & FOLLOW-UP ===
  decisionMaker: z
    .enum(['alone', 'boss', 'partners', 'committee'])
    .nullable()
    .describe(
      'Who decides: alone (solo), boss (supervisor), partners (co-owners), committee (board)',
    ),

  sendOver: z
    .array(z.enum(['Avails', 'Panel Info', 'Planning Rates']))
    .nullable()
    .describe(
      'Materials REP will send - include ALL mentioned in conversation',
    ),
  ...
  notes: z
    ...
    .nullable()
    .describe(
      'KEY DETAILS from this conversation: important points, specific requests, concerns, timeline details, follow-up items. Focus on actionable info for the sales rep.',
    ),
