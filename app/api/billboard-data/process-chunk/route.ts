...
function hasUsefulData(record: CSVRow): boolean {
  const hasViews =
    record['Avg Daily Views'] && record['Avg Daily Views'].trim() !== ''
  const hasFourWeekRange =
    record['4-Wk Range'] && record['4-Wk Range'].trim() !== ''
  const hasMarket = record.Market && record.Market.trim() !== ''
  const hasDetails = record.Details && record.Details.trim() !== ''
  const hasGeneralRange =
    record['General Range'] && record['General Range'].trim() !== ''
  const hasMarketRange =
    record['Market Range'] && record['Market Range'].trim() !== ''
  const hasPricing =
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(
      `📊 Chunk ${chunkIndex + 1}: ${chunk.length} rows → ${filteredChunk.length} useful (skipped ${skipped} empty rows)`,
    )
    ...
    for (let i = 0; i < filteredChunk.length; i += EMBEDDING_BATCH_SIZE) {
      ...
      const dataWithEmbeddings = batch.map((record, idx) => ({
        ...
        avgStatBullViewsPerWeek: parseInt(
          record['Avg Stat Bull Views/Wk'] || '0',
        ),
        avgPosterPricePerMonth: parseInt(record['Avg Poster Price/Mo'] || '0'),
        avgPosterViewsPerWeek: parseInt(record['Avg Poster Views/Wk'] || '0'),
        avgDigitalPricePerMonth: parseInt(
          record['Avg Digital Price/Mo'] || '0',
        ),
    ...
    for (let k = 0; k < processedData.length; k += INSERT_BATCH_SIZE) {
      ...
      console.log(
        `  ✓ Upserted ${k + insertBatch.length}/${processedData.length}`,
      )
    }
