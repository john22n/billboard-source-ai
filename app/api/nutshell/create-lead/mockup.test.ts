import { afterEach, beforeEach, expect, it, vi } from 'vitest'
import { NextRequest } from 'next/server'

vi.mock('@/lib/auth', () => ({
  getSession: async () => ({
    userId: 'rep',
    email: 'rep@example.test',
    sessionStartedAt: 123,
  }),
}))
vi.mock('@/lib/dal', () => ({ upsertNutshellLead: vi.fn() }))
vi.mock('@/lib/config', () => ({
  serverConfig: { nutshell: { requireApiKey: () => 'test-only' } },
  isMissingConfig: () => false,
  configErrorResponseBody: () => ({}),
}))
vi.mock('@/lib/mockup/receipts', () => ({ verifyImage: vi.fn() }))
vi.mock('@/lib/rate-limit', () => ({
  rateLimit: async () => ({ allowed: true }),
}))
import { POST as create } from './route'

const image = {
  id: '12345678-1234-4123-8123-123456789abc',
  advertiser: 'Alpine Dental',
  dataUrl: 'data:image/jpeg;base64,/9j/2Q==',
  receipt: 'signed',
}
beforeEach(() => {
  vi.spyOn(console, 'log').mockImplementation(() => {})
})
afterEach(() => {
  vi.restoreAllMocks()
  vi.unstubAllGlobals()
})

it.each([200, 503])(
  'creates a new lead and attaches only to that lead (upload status %s)',
  async (uploadStatus) => {
    const methods: string[] = []
    let uploads = 0
    let files: unknown[] = []
    vi.stubGlobal(
      'fetch',
      vi.fn(async (url, init) => {
        if (String(url).includes('/file/api/')) {
          uploads++
          return new Response('', { status: uploadStatus })
        }
        const { method, params } = JSON.parse(init.body)
        methods.push(method)
        const responses: Record<string, unknown> = {
          findUsers: [{ id: 1, emails: ['rep@example.test'] }],
          searchUniversal: { accounts: [{ id: 2, name: 'Alpine Dental' }] },
          newContact: { id: 3 },
          newSource: { id: 3 },
          newLead: { id: 42 },
        }
        let result = responses[method] ?? []
        if (method === 'getLead' || method === 'editLead') {
          expect(params.leadId).toBe(42)
          if (method === 'editLead')
            files = [
              {
                id: 9,
                entityType: 'Files',
                name: `billboard-concept-${image.id}.jpg`,
                uri: 'https://app.nutshell.com/file/api/9',
                size: 0,
              },
            ]
          result = {
            id: 42,
            rev: '1',
            primaryAccount: { name: 'Alpine Dental' },
            file: files,
          }
        }
        return Response.json({ result })
      }),
    )
    const response = await create(
      new NextRequest('http://localhost/api/nutshell/create-lead', {
        method: 'POST',
        body: JSON.stringify({
          name: 'Rep',
          entityName: 'Alpine Dental',
          phone: '3035550100',
          email: 'client@example.test',
          mockupImage: image,
        }),
      }),
    )
    expect(response.status).toBe(200)
    expect(await response.json()).toMatchObject({
      success: true,
      leadId: 42,
      imageAttachmentFailed: uploadStatus !== 200,
      message:
        uploadStatus === 200
          ? 'Lead created successfully in Nutshell'
          : 'Lead created; image could not be attached',
    })
    expect(methods.filter((method) => method === 'newLead')).toHaveLength(1)
    expect(methods.filter((method) => method === 'editLead')).toHaveLength(1)
    expect(methods.indexOf('newLead')).toBeLessThan(methods.indexOf('editLead'))
    expect(uploads).toBe(1)
  },
)
