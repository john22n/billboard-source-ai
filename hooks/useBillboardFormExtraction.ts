...
export function useBillboardFormExtraction() {
  ...
  // ✅ Compute differential updates when object changes
  ...
  // ✅ NEW: Get only the changed fields as a partial object (for store updates)
  const getChangedFieldsData = useCallback((): Partial<BillboardFormData> => {
    if (!object) return {};
  ...
  return {
    // ✅ Return accumulated form data instead of raw streaming object
    formData: isCleared ? null : accumulatedFormData,
    
    // ✅ Also expose the raw streaming object if needed for debugging
    ...
    // ✅ Expose changed fields tracking
    changedFields,
    wasFieldUpdatedByAI,
    clearChangedFieldsTracking,
    
    // ✅ NEW: Get only changed fields for store updates
